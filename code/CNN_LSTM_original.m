...
load('data_n_original.mat');load('data_pafn_original.mat');load('label_n.mat');load('label_pafn.mat');
%把数据集划分成训练集和测试集
...
trainImages=cat(1,data_n_original,data_pafn_original);
...
layers = [ ...
    sequenceInputLayer([1 1280 1],'Name','input')     %sequence输入
    sequenceFoldingLayer('Name','fold')
    % CNN特征提取
    convolution2dLayer([1 5],8,'Name','conv1')
    %batchNormalizationLayer
    reluLayer('Name','relu1')
    
    maxPooling2dLayer([1 2],'Stride',[1 2],'Name','pool1')
    
    convolution2dLayer([1 5],16,'Name','conv2')
    %batchNormalizationLayer
    reluLayer('Name','relu2')    
    
    maxPooling2dLayer([1 2],'Stride',[1 2],'Name','pool2')
    
    convolution2dLayer([1 5],32,'Name','conv3')
    %batchNormalizationLayer
